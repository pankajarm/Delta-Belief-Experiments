#!/bin/bash
### Example evaluation script for guess my city
...
: "${EXPERIMENT_NAME:=eval_cia_gmc}" && export EXPERIMENT_NAME
...
PYTHONUNBUFFERED=1 python3 -m train \
    seed="$SEED" \
    multi_turn.env="guess_my_city" \
    multi_turn.debug=true \
    multi_turn.val.n=8 \
    multi_turn.verify_judge.enabled=true \
    multi_turn.verify_judge.methods="[exact, multiple_bets, multiple_questions]" \
    data.val_files=delta_belief_rl/env/guess_my_city/direct/paprika/test.parquet \
