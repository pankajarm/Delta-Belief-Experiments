...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        limit_mm_per_prompt = {}  # None TODO check vllm version, if 10.2 set to {} else set to None
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self.lora_kwargs:
            ...
            if len(lora_int_ids) > 0:
                ...
                logger.info(
                    f"[vLLMRollout] Using LoRA adapter from vLLM: {lora_int_id}"
                )
            else:
                logger.warning(
                    f"[vLLMRollout] LoRA enabled but no adapters loaded in vLLM! Available IDs: {lora_int_ids}"
                )
