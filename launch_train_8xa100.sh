#!/bin/bash
### DeltaBelief-RL training on 8x A100 80GB — paper exact hyperparameters
### ONLY changes vs paper qwen1.7b.sh: GPU allocation (8 GPUs, judge TP=4) and wandb config
...
: "${EXPERIMENT_NAME:=train_8xa100_paper}" && export EXPERIMENT_NAME
...
PYTHONUNBUFFERED=1 python3 -m train \
    ...
    judge_rollout.ngpus=6 \
    judge_rollout.rollout.tensor_model_parallel_size=2 \
    ...
    judge_rollout.rollout.max_num_batched_tokens=4096 \
    judge_rollout.rollout.gpu_memory_utilization=0.65 \
    +judge_rollout.rollout.kv_cache_memory_bytes=30000000000 \
